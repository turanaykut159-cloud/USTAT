...
TEST_007'daki OĞUL pipeline hatasının (2781/2800) düzeltilmesi.
WATCHED_SYMBOLS override + bilinmeyen sembol için boş DataFrame.
...
class MockBridge3:
    ...
    def send_order(self, symbol, direction, lot, price,
        ...
        logger.info(f"[T007] Emir açıldı: {symbol} {direction} {lot:.1f} lot @ {entry:.2f}")
        ...
        for p in self._positions:
            ...
            if p["type"] == 0:
                ...
                if p["sl"] > 0 and cur <= p["sl"]:
                    ...
                    logger.info(f"[T007] SL: #{p['ticket']} {p['symbol']} BUY K/Z={pnl:+.2f}")
                    continue
                if p["tp"] > 0 and cur >= p["tp"]:
                    ...
                    logger.info(f"[T007] TP: #{p['ticket']} {p['symbol']} BUY K/Z={pnl:+.2f}")
                    continue
            else:
                cur = tick["ask"]
                ...
                if p["sl"] > 0 and cur >= p["sl"]:
                    ...
                    self._sl_close_count += 1
                    p["_pnl"] = pnl; p["_reason"] = "SL_HIT"; closed.append(p["ticket"])
                    logger.info(f"[T007] SL: #{p['ticket']} {p['symbol']} SELL K/Z={pnl:+.2f}")
                    continue
                if p["tp"] > 0 and cur <= p["tp"]:
                    ...
                    self._tp_close_count += 1
                    p["_pnl"] = pnl; p["_reason"] = "TP_HIT"; closed.append(p["ticket"])
                    logger.info(f"[T007] TP: #{p['ticket']} {p['symbol']} SELL K/Z={pnl:+.2f}")
                    continue
        if closed:
            for p in self._positions:
    ...
    def _record_close(self, pos):
        ...
        try:
            ...
            logger.warning(f"[T007] DB kayıt hatası: {e}")
...
def run_test():
    """Ana test akışı."""
    print("\n" + "=" * 78)
    print("  TEST_007 — XU30 + THYAO + AKBNK Tam Katman Testi")
    print("=" * 78)
    ...
    sim_dir = tempfile.mkdtemp(prefix="ustat_t007_")
    config._data.setdefault("database", {})["path"] = os.path.join(sim_dir, "t007.db")
    db = Database(config)
    ...
    for day_i, sim_day in enumerate(bdays):
        ...
        for c in range(CYCLES_PER_DAY):
            ...
            try:
                ...
                if cycle_idx <= 3:
                    import traceback
                    logger.error(f"[T007] Top5 hata: {_top5_exc}")
                    traceback.print_exc()
            ...
            try:
                ...
                if metrics["ogul"]["errors"] <= 3:
                    import traceback
                    logger.error(f"[T007] OĞUL hata #{metrics['ogul']['errors']}: {_ogul_exc}")
                    traceback.print_exc()
    ...
    json_path = os.path.join(out_dir, "TEST_007_results.json")
