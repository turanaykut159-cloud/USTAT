"""TEST_006 — XU30 + THYAO + AKBNK Tam Katman Testi (01.01.2026 →)

3 sembol, tüm katmanlar (BABA, OĞUL, H-Engine, Manuel Motor, ÜSTAT).
55 iş günü simülasyonu (01.01.2026 → 21.03.2026).
...
class PriceGen3:
    ...
    def __init__(self):
        ...
        self._cycle_count = 0
        self._trend_dir = {
            "F_THYAO": 0.00015,   # hafif yukarı (275 → 291)
            "F_AKBNK": 0.00012,   # hafif yukarı (33.50 → 35.34)
            "F_XU030": 0.00020,   # yukarı trend (13200 → 14700)
        }
...
class MockBridge3:
    ...
    def send_order(self, symbol, direction, lot, price,
        ...
        logger.info(f"[T006] Emir açıldı: {symbol} {direction} {lot:.1f} lot @ {entry:.2f}")
        ...
        for p in self._positions:
            ...
            if p["type"] == 0:
                ...
                if p["sl"] > 0 and cur <= p["sl"]:
                    ...
                    logger.info(f"[T006] SL: #{p['ticket']} {p['symbol']} BUY K/Z={pnl:+.2f}")
                    continue
                if p["tp"] > 0 and cur >= p["tp"]:
                    ...
                    logger.info(f"[T006] TP: #{p['ticket']} {p['symbol']} BUY K/Z={pnl:+.2f}")
                    continue
            else:
                cur = tick["ask"]
                ...
                if p["sl"] > 0 and cur >= p["sl"]:
                    ...
                    self._sl_close_count += 1
                    p["_pnl"] = pnl; p["_reason"] = "SL_HIT"; closed.append(p["ticket"])
                    logger.info(f"[T006] SL: #{p['ticket']} {p['symbol']} SELL K/Z={pnl:+.2f}")
                    continue
                if p["tp"] > 0 and cur <= p["tp"]:
                    ...
                    self._tp_close_count += 1
                    p["_pnl"] = pnl; p["_reason"] = "TP_HIT"; closed.append(p["ticket"])
                    logger.info(f"[T006] TP: #{p['ticket']} {p['symbol']} SELL K/Z={pnl:+.2f}")
                    continue
        if closed:
            for p in self._positions:
    ...
    def _record_close(self, pos):
        ...
        try:
            ...
            logger.warning(f"[T006] DB kayıt hatası: {e}")
...
def run_test():
    """Ana test akışı."""
    print("\n" + "=" * 78)
    print("  TEST_006 — XU30 + THYAO + AKBNK Tam Katman Testi")
    print("=" * 78)
    ...
    sim_dir = tempfile.mkdtemp(prefix="ustat_t006_")
    config._data.setdefault("database", {})["path"] = os.path.join(sim_dir, "t006.db")
    db = Database(config)
    ...
    json_path = os.path.join(out_dir, "TEST_006_results.json")
