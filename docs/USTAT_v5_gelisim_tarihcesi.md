...
| engine/top5_selection.py | Yeşil | EXPIRY_NO_NEW_TRADE_DAYS=1→0, EXPIRY_CLOSE_DAYS=1→0 |
...
| ustat_agent.py | Yeşil | Windows Service desteği + auto-install + shutdown.signal |
| start_agent.vbs | Yeşil | VBS watchdog loop (crash recovery) |
...
| Vade kısıtlaması devre dışı (BABA+Top5) | EXPIRY_DAYS=2, CLOSE/NO_NEW_TRADE=1 |
| Ajan Windows Service modu | Ajan manuel başlatma zorunluluğu |
| Crash restart (10sn/3 deneme) | — |
| Bilgisayar açılışında otomatik başlama | — |
