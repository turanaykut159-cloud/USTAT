...
| **CI-02** | AX-3 | **Kill-switch monotonluğu (+ rejim istisnası, #256 OP-H):** Seviye sadece yukarı eskale (NONE→L1→L2→L3). Otomatik düşme YOK — operator manuel ACK şart. Günlük sıfırlama sayaçları sıfırlar ama KS seviyesi korur (AX-3 monotonluk). İstisna: OLAY rejimi L2 (reason="olay_regime") rejim lifecycle'a bağlı, regime_type değişimi ile otomatik. | `baba._activate_kill_switch`, `baba._clear_kill_switch`, `baba._reset_daily` |
...
| **CI-04** | AX-5 | **EOD 17:45 Zorunlu Kapanış (motor-spesifik, #251 OP-F):** OGUL otomatik pozisyonlar zorla kapatılır. Hibrit pozisyonlar default kapatılır ancak kullanıcı explicit "yarına bırak" seçerse overnight istisna kaydı ile devam eder (`hybrid_eod` notification). Manuel pozisyonlar: operatör bilinci, bildirim + DB kaydı. Orphan: BABA raporu. | `ogul._check_end_of_day`, `_verify_eod_closure`, `h_engine.run_cycle` (EOD notification bloğu) |
...
| **CI-07** | AX-1 | **Çağrı sırası (ana döngü + ManuelMotor istisna, #252 OP-G):** Ana döngü sabit sırası `heartbeat → data → BABA → risk_check → OĞUL → H-Engine → ÜSTAT` değiştirilemez. ManuelMotor 5. motordur, on-demand çalışır (UI/API tetiği), ana döngüye girmez; ancak `BABA.check_risk_limits` kapısından geçer ve `_check_end_of_day` istisnalarına tabidir. | `main._run_single_cycle`, `manuel_motor.open_manual_trade` |
